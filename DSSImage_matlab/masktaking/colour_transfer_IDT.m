...
for i=1:nb_channels
    D0(i,:) = reshape(I0(:,:,i), 1, size(I0,1)*size(I0,2));% 3*Ic像素数
    D1(i,:) = reshape(I1(:,:,i), 1, size(I1,1)*size(I1,2));% 3*Is像素数
end
...
for i=2:nb_iterations
      R{i} = R{1} * orth(randn(3,3));%正交矩阵    旋转矩阵【10个6*3】
end

%for i=2:nb_iterations, R{i} = R{1}; end
%% pdf transfer
DR = pdf_transfer(D0, D1, R,I0, 1);
