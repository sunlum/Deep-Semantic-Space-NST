...
%
function [DR] = pdf_transfer(D0, D1, Rotations,I0, varargin)
...
for it=1:nb_iterations
    ...
    IR = I0;
    for i=1:3
        IR(:,:,i) = reshape(D0(i,:), size(IR, 1), size(IR, 2));
    end
    %D0_regrain=regrain(I0,IR);
%     imwrite(IR,strcat('outputs/',num2str(it),'_temp.jpg'));
...
function f = pdf_transfer1D(pX,pY)
    ...
    f = interp1(PY, 0:nbins-1, PX, 'linear');%重点求Tei
    f(PX<=PY(1)) = 0;
    f(PX>=PY(end)) = nbins-1;
    if sum(isnan(f))>0
