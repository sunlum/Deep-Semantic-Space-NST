...
class Optimizer(object):
    ...
    def optimize(self, x, model):
        ...
        x, min_val, info = fmin_l_bfgs_b(
            evaluator.loss, x.flatten(),  #go to ModelEvaluator.loss(x)
            fprime=evaluator.grads, maxfun=20, maxiter=20, #go to ModelEvaluator.grads(x)
            factr=1e7,
            m=4,
            bounds=data_bounds,
            iprint=0)# fmin_l_bfgs_b is slowly; evaluator.loss() is quicker
