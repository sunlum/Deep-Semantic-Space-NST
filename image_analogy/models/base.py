...
class BaseModel(object):
    ...
    def build_loss(self, a_sem_image, a_image, b_sem_image,b_image):
        ...
        loss += self.args.tv_weight * total_variation_loss(self.net_input, *b_sem_image.shape[2:])
        return loss

    def precompute_static_features(self, a_sem_image, a_image, b_sem_image,b_image):
        # figure out which layers we need to extract
        a_sem_layers, a_layers, b_sem_layers,b_layers = set(), set(), set(), set()
        if self.args.analogy_weight:
            for layerset in (a_sem_layers, a_layers, b_sem_layers,b_layers):
        ...
        if self.args.neural_style_weight:
            a_layers.update(self.args.neural_style_layers)
        # let's get those features
        all_asem_features = self.get_features(a_sem_image, a_sem_layers)
        all_a_image_features = self.get_features(a_image, a_layers)
        all_b_image_features = self.get_features(b_image, b_layers)
        all_bsem_features = self.get_features(b_sem_image, b_sem_layers)
        return all_asem_features, all_a_image_features, all_bsem_features, all_b_image_features

    def get_features(self, x, layers):
        if not layers:
            return None
        f = K.function([self.net_input], [self.get_layer_output(layer_name) for layer_name in layers])
        feature_outputs = f([x])#use [x]current input image to calculate conv3_1,4_1
