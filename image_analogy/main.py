...
import time
import sys
sys.path.append("/Users/chenjiayi/DSS-basedStyleTransfer/")
sys.path.append("/Users/chenjiayi/DSS-basedStyleTransfer/image_analogy")
sys.path.append("/Users/chenjiayi/DSS-basedStyleTransfer/image_analogy/losses")
...
def main(args, model_class):
    ...
    full_asem_image = img_utils.load_image(args.asem_image_path)#style_semlight image
    ...
    full_bsem_image = img_utils.load_image(args.bsem_image_path)#content_semlight image
    # calculate the output size
    full_img_width, full_img_height = calculate_image_dims(args, full_bsem_image)
    ...
    for scale_i in range(args.num_scales):
        ...
        if args.a_scale_mode == 'match':
            ...
            a_img_width = full_asem_image.shape[1] * scale_factor * b_scale_ratio_width
            a_img_height = full_asem_image.shape[0] * scale_factor * b_scale_ratio_height
        a_img_width = int(round(args.a_scale * a_img_width))
        a_img_height = int(round(args.a_scale * a_img_height))
        # prepare images for use
        a_sem_image = img_utils.preprocess_image(full_asem_image, a_img_width, a_img_height)
        a_image = img_utils.preprocess_image(full_a_image, a_img_width, a_img_height)
        b_sem_image = img_utils.preprocess_image(full_bsem_image, img_width, img_height)
        ...
        model.build(a_sem_image, a_image, b_sem_image,b_image, (1, img_num_channels, img_height, img_width))
        ...
        for i in range(args.num_iterations_per_scale):
            ...
            x, min_val, info = optimizer.optimize(x, model) # update (go to evaluator, loss, grads calculation)
