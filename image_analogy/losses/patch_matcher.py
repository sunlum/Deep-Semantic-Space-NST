...
def make_patch_grid(x, patch_size, patch_stride=1):#imag=B -seg
    '''x shape: (num_channels, rows, cols)'''
    x = x.transpose(2, 1, 0)#[256 32 32] -> [32 32 256]'s [1 1 256]
    ...
    patches = patches.transpose((0, 1, 4, 2, 3))#patches are [32 32 256]'s [1 1 1] sorting
...
class PatchMatcher(object):
    ...
    def update_with_patches(self, input_patches, reverse_propagation=False):#input patches = B-patches
        self._propagate(input_patches, reverse_propagation=reverse_propagation)
        self._random_update(input_patches)

    def get_patches_for(self, img):#img=B -seg, self is A -seg
    ...
    def take_best(self, coords_a, similarity_a, coords_b, similarity_b):
        delta_similarity = similarity_a - similarity_b#[32 32 256]
    ...
    def lookup_coords(self, x, coords):
        ...
        return x[i_coords[0], i_coords[1]]#F_style's patches = x  , i_coords = round (p->q)

    def get_reconstruction(self, patches=None, combined=None):
        if combined is not None:
            patches = make_patch_grid(combined, self.patch_size)#FA'prime - style img
        if patches is None:
            patches = self.target_patches
        patches = self.lookup_coords(patches, self.coords)#re-order the order of pixels(patches)
