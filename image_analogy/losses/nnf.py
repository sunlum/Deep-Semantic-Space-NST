...
def nnf_analogy_loss(a, a_prime, b, b_prime, num_steps=5, jump_size=1.0, patch_size=1, patch_stride=1):
    ...
    #Define a PatchMatcher object--[256,32,32], coords=A(mask)

    b_patches = matcher.get_patches_for(b)
    b_normed = matcher.normalize_patches(b_patches)
    for i in range(num_steps):
        # 15 times to update A's coords (similarity using [1 1 1])
        matcher.update_with_patches(b_normed, reverse_propagation=bool(i % 2))

    target = matcher.get_reconstruction(combined=a_prime)#let F_target(q=p: FA.coords=[ FA mapping FB]) = FA'style(p)
    loss = content_loss(target, b_prime)# let FX to approach the reconstructed F_target
    return loss


class NNFState(object):
    def __init__(self, matcher, f_layer):#match is belonging to style image (F_A');  f_layer is F_x
        self.matcher = matcher
        mis = matcher.input_shape
        self.placeholder = K.placeholder(mis[::-1])
        self.f_layer = f_layer

    def update(self, x, num_steps=5):# excuted by eval_loss_and_grads()
    #
        x_f = self.f_layer([x])[0] # calculate F(current x)
        x_patches = self.matcher.get_patches_for(x_f[0])
        x_normed = self.matcher.normalize_patches(x_patches)
        for i in range(num_steps): # update coords(F_s->F_current-x) (matcher belongs to F_style)
